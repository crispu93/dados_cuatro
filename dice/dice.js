...
(function(dice) {
    ...
    this.standart_d20_dice_face_labels1 = [' ', '0', '4', '4', '4', '4', '0', '0'];
    this.standart_d20_dice_face_labels2 = [' ', '0', '3', '3', '3', '3', '3', '3'];
    this.standart_d20_dice_face_labels3 = [' ', '0', '2', '2', '2', '6', '6', '2'];
    this.standart_d20_dice_face_labels4 = [' ', '0', '5', '5', '5', '1', '1', '1']; 
    this.standart_d40_dice_face_labels = [' ', '00', '10', '20', '30', '40', '50',
            '60', '70', '80', '90'];
    ...
    this.create_d1_materials = function(size, margin, labels) {
        function create_d1_text(text, color, back_color) {
            var canvas = document.createElement("canvas");
            var context = canvas.getContext("2d");
        ...
        for (var i = 0; i < labels.length; ++i)
            materials.push(new THREE.MeshPhongMaterial($t.copyto(this.material_options,
                        { map: create_d1_text(labels[i], this.label_color, this.dice_color) })));
        return materials;
    }

    this.create_d1_geometry = function(radius) {
    ...
    this.create_d2_geometry = function(radius) {
        ...
        return create_geom(vertices, faces, radius, 0.1, 5*Math.PI/4 , 0.96);
    }
    ...
    this.known_types = ['d1', 'd2', 'd3', 'd4'];
    this.dice_face_range = { 'd1': [1, 4], 'd2': [1, 6], 'd3': [1, 8], 'd4': [0, 9], 
        'd12': [1, 12], 'd20': [1, 20], 'd40': [0, 9] };
    this.dice_mass = { 'd1': 300, 'd2': 300, 'd3': 340, 'd4': 350, 'd12': 350, 'd20': 400, 'd40': 350 };
    this.dice_inertia = { 'd1': 5, 'd2': 13, 'd3': 10, 'd4': 9, 'd12': 8, 'd20': 6, 'd40': 9 };
    ...
    this.create_d40 = function() {
        if (!this.d4_geometry) this.d4_geometry = this.create_d4_geometry(this.scale * 0.9);
        if (!this.d40_material) this.d40_material = new THREE.MeshFaceMaterial(
                this.create_dice_materials(this.standart_d40_dice_face_labels, this.scale / 2, 1.5));
        return new THREE.Mesh(this.d4_geometry, this.d40_material);
    }
    ...
    function get_dice_value(dice) {
        var vector = new THREE.Vector3(0, 0, dice.dice_type == 'd1' ? -1 : 1);
        ...
        if (dice.dice_type == 'd40') matindex *= 10;
        if (dice.dice_type == 'd4' && matindex == 0) matindex = 10;
        return matindex;
    }
    ...
    function shift_dice_faces(dice, value, res) {
        ...
        if (dice.dice_type == 'd4' && value == 10) value = 0;
        ...
        if (dice.dice_type == 'd1' && num != 0) {
            if (num < 0) num += 4;
            dice.material = new THREE.MeshFaceMaterial(
                    that.create_d1_materials(that.scale / 2, that.scale * 2, d1_labels[num]));
        }
        dice.geometry = geom;
    }
    ...
    this.dice_box.prototype.draw_selector = function() {
        ...
        for (var i = 0, pos = -3; i < that.known_types.length; ++i, pos+=2) {
